        ...
        !(function () {
            ...
            t.querySelectorAll("tbody tr").forEach((t) => {
                const e = t.querySelectorAll("td"),
                    a = moment(e[1].innerHTML, "MMM D, YYYY").format("x");
            ...
            a.addEventListener("change", function (t) {
                ...
                r.addEventListener("change", function (t) {
                    switch (t.target.value) {
                        case "thisyear":
                            (n = moment().startOf("year").format("x")),
                                (o = moment().endOf("year").format("x")),
                                e.draw();
                            break;
                        case "thismonth":
                            (n = moment().startOf("month").format("x")),
                                (o = moment().endOf("month").format("x")),
                                e.draw();
                            break;
                        case "lastmonth":
                            (n = moment()
                            ...
                            break;
                        case "last90days":
                            (n = moment().subtract(30, "days").format("x")),
                                (o = moment().format("x")),
                                e.draw();
                            break;
                        default:
                ...
                $.fn.dataTable.ext.search.push(function (t, e, a) {
                    var r = n,
                        m = o,
                        s = parseFloat(moment(e[1]).format("x")) || 0;
                    return !!(
                        (isNaN(r) && isNaN(m)) ||
                        (isNaN(r) && s <= m) ||
                        (r <= s && isNaN(m)) ||
